...
int main()
{
  ...
  while (1)
  {
    ...
    if (linha[len - 1] == '\n')
      linha[len - 1] = '\0';
    int argn = parse(linha, args); /* particiona a string em argumentos */

    if (!builtin(args, argn))
      execute(argn, args); /* executa o comando */
  }
  return 0;
}
...
int builtin(char **args, int argn)
{
  ...
  if (0 == strcmp(args[0], "calc") && argn == 4)
  {
    calc(args[1], args[2], args[3]);
    return 1; // comando embutido
  }
  if (0 == strcmp(args[0], "bits") && (argn == 4 || (0 == strcmp(args[2], "~") && argn == 3)))
  {
